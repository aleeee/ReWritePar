...
public class Model3 {
	...
	public Model3(SkeletonPatt skeletonPatt, int maxParDegree)
			throws IloException {
		super();
		...
		cplex.exportModel("C:\\Users\\me\\Desktop\\out\\cplexModel"+skeletonPatt.hashCode()+".lp");

		cplex.setOut(null);
	}

	private void addVariables() {
		g.getChildren().stream().filter(s -> s instanceof FarmPatt).forEach(varCreator);
	...
	private void addConstraints() throws IloException {
		try {
			...
			for(SkeletonPatt v : g.getChildren()){
				if(!(v instanceof FarmPatt))continue;
				List<IloNumVar> vars = variables.get(v);

				expr = cplex.sum(expr, vars.get(1));

				constraints.add(cplex.addGe(cplex.prod(vars.get(0), vars.get(1)), v.getChildren().get(0).getIdealServiceTime()));
			}
	...
	private void addObjective() throws IloException {
		...
		for(SkeletonPatt sp : g.getChildren()) {
			if(!(sp instanceof FarmPatt))continue;
	...
	public List<SkeletonPatt> getSolutions() throws IloException{
		...
		for (SkeletonPatt p :g.getChildren()){
			if(!(p instanceof FarmPatt))continue;
			List<IloNumVar> vars = variables.get(p);
			System.out.println(p + " vars.get(0) " +vars.get(0) +" vars.get(1) "+vars.get(1));
