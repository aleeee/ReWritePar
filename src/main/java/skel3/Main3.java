...
import graph.SkGraph;
import graph.SkGraph.Edge;
import graph.SkGraph;
...
//		mcts.selectAction(n);
		...
		SkGraph dg = new  SkGraph();
		dg.axpandAndSearch(n, 4);
...
//				.forEach(sk -> System.out.println(sk + "\t" + sk.getServiceTime()));
	 ...
	 private static void renderHrefGraph(SkGraph dg_)
...
//		 dg.g.removeAllVertices( dg.g.vertexSet().stream().filter( v ->  v.getPatterns() == null).collect(Collectors.toList()));
		        ...
		        ComponentNameProvider<SkGraph.Edge> edgeLabelProvider = (SkGraph.Edge sk) -> {return sk.getRule() != null? sk.getRule().toString():
		        													"NO LABLE";}; 
...
//				{return null;};
				  GraphExporter<SkeletonPatt, SkGraph.Edge> exporter =//new JSONExporter<SkeletonPatt, Edge>(vertexIDProvider, vertexAttributeProvider, edgeIDProvider, edgeAttributeProvider);
		        		new DOTExporter<SkeletonPatt, SkGraph.Edge>(vertexIdProvider, vertexLabelProvider, edgeLabelProvider);
...
//					CGraphviz..VizjsOptions();
	 ...
	 private static void exportJson(SkGraph dgJson)  throws ExportException, IOException
		    {	
...
//				ComponentNameProvider<Edge> edgeIDProvider = (Edge e) -> String.valueOf(id.incrementAndGet());
				...
				ComponentAttributeProvider<Edge> edgeAttributeProvider = new ComponentAttributeProvider<SkGraph.Edge>() {
