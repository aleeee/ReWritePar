 ...
 */
public class ViewTree extends JComponent {
    SkeletonPatt root;
    int nw = 30;
    int nh = 20;
    int inset = 20;
    ...
    HashMap<SkeletonPatt, Color> high;

    public ViewTree(SkeletonPatt root) {
        this.root = root;
        high = new HashMap<SkeletonPatt, Color>();
    }
    ...
    private void draw(Graphics2D g, SkeletonPatt cur, int x, int y, int wFac) {
        // draw this one, then it's children
        if(cur == null)
        	return;
        int arity = (cur.getPatterns() != null ? cur.getPatterns().size(): 1);
        System.out.println("arity " + arity);
        for (int i = 0; i < arity; i++) {
            if (cur.getPatterns() != null ) {
                int xx = (int) ((i + 1.0) * wFac / (arity + 1) + (x - wFac / 2));
                int yy = y + heightPerLevel;
                g.setColor(fg);
                g.drawLine(x, y, xx, yy);
                System.out.println("view " + cur.getPatterns().get(i));
                draw(g,cur.getPatterns().get(i), xx, yy, wFac / arity);
            }
        }
        drawNode(g, cur, x, y);
    }

    private void drawNode(Graphics2D g, SkeletonPatt node, int x, int y) {
        String s = node.getLable(); //(int) node.totValue + "/" + (int) node.nVisits;
        g.setColor(nodeBg);
    ...
    public ViewTree showTree(String title) {
